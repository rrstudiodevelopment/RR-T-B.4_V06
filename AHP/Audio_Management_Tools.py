...
class RAHA_OT_goto_vse(Operator):
    ...
    def execute(self, context):
        ...
        # Coba pindah ke workspace "Video Editing" jika sudah ada
        if "Video Editing" in bpy.data.workspaces:
            window.workspace = bpy.data.workspaces["Video Editing"]
        else:
            # Buat workspace baru
            new_workspace = bpy.data.workspaces.new(name="Video Editing")
            screen = new_workspace.screens[0]

            # Tambahkan area VSE
            for area in screen.areas:
                screen.areas.remove(area)

            area = screen.areas.new('SEQUENCE_EDITOR')
            area.spaces.active.view_type = 'SEQUENCER'
            window.workspace = new_workspace
            self.report({'INFO'}, "Video Editing workspace created.")
